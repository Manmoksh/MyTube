...
const userVariantInfo = cva("flex item-center gap-1", {
  variants: {
    size: {
...
interface UserInfoProps extends VariantProps<typeof userVariantInfo> {
  name: string;
  className?: string;
}
export const UserInfo = ({ name, className, size }: UserInfoProps) => {
  return (
    <div className={cn(userVariantInfo({ size, className }))}>
      <Tooltip>
